...
namespace CloudYourself.Backend.Gateways.SelfServicePortalApi.Infrastructure
{
    ...
    public abstract class ProxyController : HypermediaController
    {
        ...
        protected async Task<string> ProxyPostAsync(string baseAddress = null, string url = null)
        {
            ...
            {
                ThrowResponseErrorException(responseContent);
            }
        ...
        /// <summary>
        /// Throws the response error exception.
        /// </summary>
        /// <param name="responseContent">Content of the response.</param>
        private void ThrowResponseErrorException(string responseContent)
        {
            JsonElement errorObject = JsonDocument.Parse(responseContent).RootElement;

            string errorCode = "Unknown";
            JsonElement errorCodeElement;
            if (errorObject.TryGetProperty("errorCode", out errorCodeElement))
            {
                errorCode = errorCodeElement.GetString();
            }

            string errorMessage = null;
            JsonElement errorMessageElement;
            if (errorObject.TryGetProperty("errorMessage", out errorMessageElement))
            {
                errorMessage = errorMessageElement.GetString();
            }

            throw new ProxyResponseException(errorCode, errorMessage);
        }
    }
}
